...
In the SIR model, \(\gamma\) denotes the recovery rate. In the manuscript, the mobility coefficient is denoted as \(\xi(t)\).

Some older local outputs or script versions may use file names such as `Seoul_gamma.csv` for mobility-factor outputs. In that context, `gamma` refers to the mobility coefficient file generated by the code, not to the SIR recovery rate. For clarity, new outputs should preferably use names such as `Seoul_xi.csv`.

### Optional implementation note on elasticity settings

Some scripts in this repository include optional sensitivity settings with a mobility elasticity parameter \(\theta\), including:

- `0.01`
- `0.5`
- `1.0`

These implementation settings should be distinguished from the main manuscript definition of the mobility coefficient, where:
...
If the current script version writes files such as `Seoul_gamma.csv`, interpret those files as mobility-coefficient outputs and not as the recovery-rate parameter \(\gamma\).
...
For the 2016-2017 season, run the model scripts for each elasticity setting:
...
## Reproducibility checklist before release

Before using this repository as the public reproducibility repository for the manuscript, check the following:

- [ ] The manuscript title in this README matches the final published title.
- [ ] `CITATION.cff` exists at the repository root.
- [ ] The author list matches the manuscript author list.
- [ ] Raw data redistribution rules have been checked.
- [ ] Processed data or reconstruction instructions are provided.
- [ ] Mobility outputs use `xi` terminology, or legacy `gamma` file names are clearly explained.
- [ ] Scripts run from a clean clone without local absolute paths.
- [ ] Output folders for all manuscript seasons are included or documented.
- [ ] The DOI and final bibliographic information are updated after publication.
...
After the article DOI is finalized, update this section and `CITATION.cff` with the final DOI, volume, issue, and page numbers.
